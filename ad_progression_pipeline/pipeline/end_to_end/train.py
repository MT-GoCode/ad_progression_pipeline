...
from ad_progression_pipeline.components.ingestion.flows import (
    categorical_ingestion,
    sequential_ingestion,
)
from ad_progression_pipeline.components.models.model_interface import ModelInterface
from ad_progression_pipeline.pipeline.helpers import (
    context_handler,
    data,
    files,
    optuna_,
...
def train() -> None:
    ...
    study = optuna.create_study(
        storage="sqlite:///" + output_dir + "study.db", direction="maximize",
    )

    @flow
    def objective(trial: optuna.trial.Trial) -> float:
        params = optuna_.suggest_hyperparameters(
            ranges=context.optuna_ranges, trial=trial,
        )
        ...
        if context.model_type == "binary":
            ...
            train_input_matrix, train_output_matrix = sequential_ingestion(
                df=train_data,
            )
            test_input_matrix, test_output_matrix = sequential_ingestion(df=test_data)
            ...
            results = model.infer_and_gather_metrics(
                train_input_matrix, train_output_matrix,
            )
