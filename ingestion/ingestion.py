...
from constants import TOTAL_VISITS, CONSTANT_COLUMNS

@dataclass
class Ingestor:

    csv_path : pd.DataFrame
    num_input_visits : int
    ...
    @cached_property
    def X(self):
        return self.df.groupby('NACCID').head(self.num_input_visits)
    
    @cached_property
    def Y(self):
        return self.df.groupby('NACCID').tail(TOTAL_VISITS - self.num_input_visits)
